...
{
    public class CoreXLuaILuaStateMachineMonoBridge : LuaBase, Core.XLua.ILuaStateMachineMono
    {
	    public static LuaBase __Create(int reference, LuaEnv luaenv)
		{
		    return new CoreXLuaILuaStateMachineMonoBridge(reference, luaenv);
		}
		
		public CoreXLuaILuaStateMachineMonoBridge(int reference, LuaEnv luaenv) : base(reference, luaenv)
        {
        }
		
        
		System.Action Core.XLua.ILuaStateMachineMono.Start()
		{
		...
		System.Action Core.XLua.ILuaStateMachineMono.Update()
		{
#if THREAD_SAFE || HOTFIX_ENABLE
            lock (luaEnv.luaEnvLock)
            {
		...
		System.Action Core.XLua.ILuaStateMachineMono.OnDestroy()
		{
#if THREAD_SAFE || HOTFIX_ENABLE
            lock (luaEnv.luaEnvLock)
            {
		...
		void Core.XLua.ILuaStateMachineMono.Dispatch(string evenName, object[] args)
		{
#if THREAD_SAFE || HOTFIX_ENABLE
            lock (luaEnv.luaEnvLock)
            {
				...
				LuaAPI.lua_getref(L, luaReference);
				LuaAPI.xlua_pushasciistring(L, "Dispatch");
				if (0 != LuaAPI.xlua_pgettable(L, -2))
				{
					luaEnv.ThrowExceptionFromError(err_func - 1);
				}
				if(!LuaAPI.lua_isfunction(L, -1))
				{
					LuaAPI.xlua_pushasciistring(L, "no such function Dispatch");
					luaEnv.ThrowExceptionFromError(err_func - 1);
				}
				LuaAPI.lua_pushvalue(L, -2);
				LuaAPI.lua_remove(L, -3);
				LuaAPI.lua_pushstring(L, evenName);
				translator.Push(L, args);
